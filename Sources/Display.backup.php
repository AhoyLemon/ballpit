...
function Display()
{
	...
	if (!empty($messages))
	{
		...
		$messages_request = $smcFunc['db_query']('', '
			SELECT
				...
				smileys_enabled, poster_name, poster_email, approved, bulbs_total,
	...
	// Bulb cleanup
	$context['can_bulb'] = !$context['user']['is_guest'];
	...
	if (!empty($messages))
	{
		$request = $smcFunc['db_query']('','
			...
			WHERE id_topic = {int:topicid}
				AND id_voter = {int:current}',
			array(
				'topicid' => $topic,
				'current' => $context['user']['id'],
			)
		);
		// $context['bulb_voted'] == 0 if user has not bulbed in topic
		if ($smcFunc['db_num_rows']($request) != 0)
			$context['bulb_voted'] = 1;
		else
			$context['bulb_voted'] = 0;
...
function prepareDisplayContext($reset = false)
{
	...
	$output = array(
		...
		'bulbs_total' => $message['bulbs_total'],'can_see_ip' => allowedTo('moderate_forum') || ($message['id_member'] == $user_info['id'] && !empty($user_info['id'])),
	);
