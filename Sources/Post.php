...
function Post2()
{
	...
	else
	{
		...
		if ($smcFunc['htmltrim'](strip_tags(parse_bbc($_POST['message'], false), '<img><iframe><video>')) === '' && (!allowedTo('admin_forum') || strpos($_POST['message'], '[html]') === false))
			$post_errors[] = 'no_message';
	}
...
function JavaScriptModify()
{
	...
	if (isset($_POST['message']))
	{
		...
		else
		{
			...
			if ($smcFunc['htmltrim'](strip_tags(parse_bbc($_POST['message'], false), '<img><video>')) === '')
			{
				$post_errors[] = 'no_message';
				unset($_POST['message']);
			}
