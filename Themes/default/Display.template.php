	...
	while ($message = $context['get_message']())
	{
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ($settings['show_profile_buttons'])
			{
				...
				if ($message['member']['can_view_profile'])
					echo '
										<li><a href="', $message['member']['href'], '">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/icons/16/profile.png" alt="' . $txt['view_profile'] . '" title="' . $txt['view_profile'] . '" />' : $txt['view_profile']), '</a></li>';

				// Don't show an icon if they haven't specified a website.
				if ($message['member']['website']['url'] != '' && !isset($context['disabled_fields']['website']))
					echo '
										<li><a href="', $message['member']['website']['url'], '" title="' . $message['member']['website']['title'] . '" target="_blank" rel="noopener noreferrer" class="new_win">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/icons/16/website.png" alt="' . $message['member']['website']['title'] . '" />' : $txt['www']), '</a></li>';

				// Any custom fields for standard placement? (STEAM FUNCTION)
				if (!empty($message['member']['custom_fields']))
				{
					foreach ($message['member']['custom_fields'] as $custom)
				...
				if (in_array($message['member']['show_email'], array('yes', 'yes_permission_override', 'no_through_forum')))
					echo '
										<li><a href="', $scripturl, '?action=emailuser;sa=email;msg=', $message['id'], '" rel="nofollow">', ($settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/icons/16/mail.png" alt="' . $txt['email'] . '" title="' . $txt['email'] . '" />' : $txt['email']), '</a></li>';

				// Since we know this person isn't a guest, you *can* message them.
				if ($context['can_send_pm'])
					echo '
										<li><a href="', $scripturl, '?action=pm;sa=send;u=', $message['member']['id'], '" title="', $message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline'], '">', $settings['use_image_buttons'] ? '<img src="' . $settings['images_url'] . '/icons/16/im_' . ($message['member']['online']['is_online'] ? 'on' : 'off') . '.png" alt="' . ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']) . '" />' : ($message['member']['online']['is_online'] ? $txt['pm_online'] : $txt['pm_offline']), '</a></li>';
		...
		if ($message['can_approve'])
			echo '
				...
				// BULBMOD CHANGE START	
				// colour the icon based on the number of bulbs received				
				if ($message['bulbs_total'] == 0)
					$bulbicon = '<li class="bulb_unlit" id = "bulb_unlit_' . $message['id'] . '">';
				elseif ($message['bulbs_total'] < 5)
					$bulbicon = '<li class="bulb_lit" id = "bulb_lit_' . $message['id'] . '">';
				else
					$bulbicon = '<li class="bulb_bright" id = "bulb_bright_' . $message['id'] . '">';					
				if ($context['can_bulb'])
				{
					if (($message['member']['id'] == $context['user']['id']) || ($context['bulb_voted'] == 1))
					{
						echo '
											', $context['bulb_voted'], $bulbicon, $message['bulbs_total'], '</li>';
					}
					else
					{
						echo '
											', $bulbicon, '<a href="', $scripturl, '?action=bulb;msg=', $message['id'], ';topic=', $context['current_topic'], '">', $message['bulbs_total'], '</a></li>';
					}			
				}
				// BULBMOD CHANGE END
		...
		if ($context['can_quote'] && !empty($options['display_quick_reply']))
			echo '
				<li>
					<a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');">
						<img src="', $settings['images_url'], '/icons/16/quote_red.png" alt="quote" title="quote this post in yours" />
					</a>
				</li>';
		...
		elseif ($context['can_quote'])
			echo '
				<li>
					<a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '">
						<img src="', $settings['images_url'], '/icons/16/quote_red.png" alt="quote" title="quote this post in yours" />
					</a>
				</li>';
			...
			echo '
				<li>
					<a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '">
						<img src="', $settings['images_url'], '/icons/16/edit_red.png" alt="edit" title="edit this post" />
					</a>
				</li>';
		...
		if ($message['can_remove'])
			echo '
				<li>
					<a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');">
						<img src="', $settings['images_url'], '/icons/16/remove_red.png" alt="remove" title="remove this post" />
					</a>
				</li>';
		...
		if ($context['can_split'] && !empty($context['real_num_replies']))
			echo '
				<li>
					<a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '">
						<img src="', $settings['images_url'], '/icons/16/split_red.png" alt="split" title="split this post" />
					</a>
				</li>';
		...
		if ($context['can_report_moderator'] && !$message['is_message_author'])
			echo '
				<li>
					<a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '">
						<img src="', $settings['images_url'], '/icons/16/bullhorn_red.png" alt="tattle" title="tattle on this post" />
					</a>
				</li>';
			...
			echo '
				<li>
					<a href="', $scripturl, '?action=profile;area=issuewarning;u=', $message['member']['id'], ';msg=', $message['id'], '">
						<img src="', $settings['images_url'], '/icons/16/cop_red.png" alt="warn" title="issue a warning because of this message" />
					</a>
				</li>';
		...
		if ($context['can_moderate_forum'] && !empty($message['member']['ip']))
			echo '
				<li>
					<a href="', $scripturl, '?action=', !empty($message['member']['is_guest']) ? 'trackip' : 'profile;area=tracking;sa=ip;u=' . $message['member']['id'], ';searchip=', $message['member']['ip'], '">
						<img src="', $settings['images_url'], '/icons/16/fingerprint.png" alt="IP" title="track this IP" />
					</a>
				</li>';
		...
		// Otherwise, you see NOTHING!
			...
			echo '
				<li>
					<img src="', $settings['images_url'], '/icons/16/fingerprint.png" title="IP logged." alt="IP logged" />
				</li>';
